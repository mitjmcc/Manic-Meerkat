...
*	
*	Mitch added random function from https://stackoverflow.com/a/10625698
*/
Shader "Custom/sufaceshaderYuan" {
	...
	SubShader{
		...
		// Replacement for function missing from MyFunc.cginc
		...
		void disp(inout appdata v)
		{
			...
			*/
			...
			if (length(v.normal * d) > _LengthCull)
				v.vertex.xyz -= rand(v.vertex.xy)*v.normal*(2 * _LengthCull + 1) / ((1 + d)*(3.5 - 2 * _LengthCull));
		}
		fixed4 _Color;
		sampler2D _MainTex;
